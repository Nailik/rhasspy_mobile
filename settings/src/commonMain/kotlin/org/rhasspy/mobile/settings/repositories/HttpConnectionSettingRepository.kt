...
class HttpConnectionSettingRepository internal constructor() : IHttpConnectionSettingRepository, KoinComponent {
    ...
    override fun addOrUpdateHttpConnection(connection: HttpConnection) {
        with(connection) {
            database.settingsHttpConnectionsQueries.insertOrUpdate(
                ...
                isHermes = if (isHermes) 1L else 0L,
                isWyoming = if (isWyoming) 1L else 0L,
                isHomeAssistant = if (isHomeAssistant) 1L else 0L,
    ...
    private fun SettingsHttpConnection.mapToHttpConnection(): HttpConnection {
        return HttpConnection(
            ...
            timeout = timeout,
            bearerToken = bearerToken,
            isHermes = isHermes == 1L,
            isWyoming = isWyoming == 1L,
            isHomeAssistant = isHomeAssistant == 1L,
